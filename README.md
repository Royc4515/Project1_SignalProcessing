...
כדי לאפיין את הדינמיקה הזמנית הפנימית ודפוסי המחזוריות של הנוירונים (A, C, D, E), חושבה פונקציית האוטוקורלציה על פני שתי הדקות הראשונות של הניסוי (חלון זמן של $120,000\,\text{ms}$). הפונקציה נורמלה באופן ריגורוזי ביחס לתוחלת המפגשים הצפויה מתהליך פואסון סטטיסטי בלתי-תלוי בעל קצב ירי זהה:
$$\text{expected\_coincidences} = \bar{\lambda}^2 \cdot N_{\text{samples}}$$

תחת נרמול זה, ערך של $1.0$ מייצג את קו הבסיס (Baseline) האקראי שבו אין כל תלות זמנית בין פוטנציאלי הפעולה. ערכים הגבוהים מ-$1.0$ מעידים על הסתברות יתר זמנית (צימוד/צרורות), וערכים הנמוכים מ-$1.0$ מעידים על דיכוי ירי.
...
1. **זיהוי וכימות התקופה הרפרקטורית (חלון ה-Zoom):**
   בבחינת הלאגים הקצרים ברזולוציה של $1\,\text{ms}$, נצפה "בור" עמוק שבו ערך האוטוקורלציה צונח ל-0 מוחלט מיד לאחר נקודת האפס. תופעה זו משקפת את **התקופה הרפרקטורית המוחלטת** (Absolute Refractory Period) – חלון הזמן הפיזיולוגי שבו תעלות הנתרן תלויות-המתח ($Na_v$) עוברות אינאקטיבציה (שינוי קונפורמציות מבני), וממברנת התא אינה מסוגלת לייצר פוטנציאל פעולה נוסף ללא קשר לעוצמת הקלט.
   - **נוירונים A ו-E:** מציגים תקופה רפרקטורית מוחלת המסתיימת במדויק ב-$3\,\text{ms}$. ערך זה תואם את הדינמיקה הביולוגית הסטנדרטית ומאפשר לתא E לירות ספייקים בצפיפות גבוהה מאוד בתוך הצרור.
   - **נוירון C:** מציג תקופה רפרקטורית ארוכה יותר המסתיימת ב-$6\,\text{ms}$, דבר המגביל את קצב הירי הרגעי המקסימלי שלו.
   - **נוירון D:** הגרף המוגדל חושף ערכי אפס עגולים לאורך כל טווח הלאגים של $0-15\,\text{ms}$. ממצא אמפירי זה מעיד על מבנה זמני קשיח במיוחד: הנוירון אינו מסוגל פיזיולוגית לירות במרווחים קצרים, והוא חוזר לירות אך ורק כאשר הוא מתקרב לזמן המחזור הטבעי שלו (סביב $20\,\text{ms}$).

2. **משמעות מספר הפיקים והמחזוריות:**
   מספר הפיקים העוקבים ותדירותם משקפים את מידת המחזוריות (Rhythmicity) של התא. עבור נוירונים אוסצילטוריים קצביים, המרחק הזמני בין פיק עוקב למשנהו באוטוקורלציה שווה בדיוק ל-ISI השכיח של התא ($1/r$). ספירת השיאים מאפשרת לחלץ את תדר האוסצילציה בצורה בלתי תלוי. בנוירון A (הפואסוני), הגרף נותר שטוח לחלוטין על ערך $1.0$ לכל אורך הלאגים (למעט בבור הרפרקטורי), מה שמוכיח מתמטית אי-תלות סטטיסטית מלאה בין ספייק עבר לספייק עתידי.

3. **המינימום המקומי (שקיעה לאחר השיאים):**
   מיד לאחר כל פיק דומיננטי, נצפית שקיעה של הגרף אל מתחת לקו הבסיס האקראי ($< 1.0$). תופעה זו מייצגת את תהליך **הדיכוי הפוסט-ספייק** (Post-spike suppression), הנובע פיזיולוגית מתהליכי היפרפולריזציה ממושכת (מונעת על ידי פתיחת תעלות אשלגן תלויות-סידן $K_{\text{Ca}}$ או זרמי כלור דרך רצפטורים אינהיביטוריים). בנוירון Bursting כמו E, המינימום העמוק והרחב המופיע מיד לאחר הפיק המרכזי מייצג את "תקופת השקט הממושכת" (Inter-burst interval) הנדרשת לתא כדי להטעין מחדש את הממברנה לפני פריצת הצרור הבא.

4. **השוואה מורפולוגית ריגורוזית — נוירון C מול נוירון D:**
   למרות ששני הנוירונים מסווגים כנוירונים מחזוריים וסדירים (Regular Oscillators), קיים שוני מורפולוגי קיצוני בצורת הפיקים שלהם:
   - **נוירון D (שעון ביולוגי קשיח):** מציג פיקים צרים מאוד, חדים וגבוהים. רוחב הפיק הצר מעיד על כך שפיזור זמני הירי סביב מחזור המטרה הוא אפסי – כלומר, ה-Jitter הזמני שואף לאפס. ממצא זה מתיישב בצורה מושלמת עם מדד ה-Fano Factor שחושב עבורו ($FF \approx 0.01$), המעיד על תהליך דטרמיניסטי לחלוטין.
   - **נוירון C (אוסצילטור רועש):** מציג פיקים רחבים, נמוכים ומורחים יותר. רוחב הפיק משקף את העובדה שישנו פיזור סטוכסטי (Jitter) סביב זמן הירי המועדף. למרות שהוא שומר על קצביות ממוצעת, הוא סובל מרעש זמני פנימי, דבר המסביר מדוע מדד פאנו שלו נמוך מ-1 אך רחוק מאפס ($FF \approx 0.15$).
---
