...
NB = 'Project1_SignalProcessing.ipynb'
OUT = 'report.html'

# Cells to skip entirely (technical setup, no academic content)
...
def render_markdown(src: str) -> str:
    ...
    # The notebook's markdown cells already wrap their content in <div dir="rtl">.
    # We need to: handle headings (###, ##, #), bold (**), italic (*), code (`),
    # tables (|...|), lists (-), and math ($...$, $$...$$) — leave math for MathJax.
    out = src
    ...
    # Italic — skip if pattern is part of math; simple heuristic: skip *...* with no math nearby
    # Leave it: too risky to mis-italicize math. Markdown italic is rarely used in this notebook.
    out = re.sub(r'`([^`]+)`', r'<code>\1</code>', out)

    # Paragraph breaks: double newline -> </p><p>
    # but only outside of <div>, <h*>, <table>, <ul>
    ...
    for p in paragraphs:
        ...
        if not p:
            continue
        # If already a block element, leave alone
...
def render_code_output(out):
    """Render an output object from a code cell."""
    pieces = []
...
<head>
<meta charset="utf-8" />
<title>Project 1 — Signal Processing</title>
<style>{CSS}</style>
<script>
  MathJax = {{
...
<div class="title-block">
  <h1>פרויקט 1 — עיבוד אותות במוח</h1>
  <div class="subtitle">Sampling, Noise Reduction, Spike Train Analysis</div>
